...
- Release workflow hung on the Vision suites it alone still ran; it now uses
  the same skip list as the Build workflow.

## [1.5.2] - 2026-08-23

### Changed

- CI runs only the Vision-free suites: both `VNRecognizeTextRequest` and
  `RecognizeDocumentsRequest` hang indefinitely on hosted macos-26-arm64
  runners (zero completions in 2.5 min, all pass in under a second on real
  hardware), so the Vision suites stay covered by local machines.
- Structured-recognition wait reduced from 120 s to 30 s — roughly 70× the
  measured per-page baseline — so a wedged backend fails fast instead of
  stacking past any job budget.

## [1.5.1] - 2026-08-23

### Fixed

- CI no longer hangs forever: the structured-recognition bridge times out
  after 120 s and reports `recognitionTimedOut` instead of blocking on a
  wedged Vision backend, and both workflows cap jobs at 20 minutes.
- Test step split into Vision-free and Vision suites so a hang names the
  culprit group in the log.
